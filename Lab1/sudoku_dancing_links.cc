...
int
Dance::get_row_col(int row, int val)
{
    return kRow+row*10+val;
}

int
Dance::get_col_col(int col, int val)
{
    return kCol+col*10+val;
}

int
Dance::get_box_col(int box, int val)
{
...
void
Dance::sudoku_dancing_links_solve(int inout[81]) 
{
    stack_.reserve(100);
    root_ = new_column();
...
void
Dance::put_left(Column* old, Column* nnew)
{
        nnew->left = old->left;
        nnew->right = old;
        old->left->right = nnew;
...
void
Dance::put_up(Column* old, Node* nnew)
    {
        nnew->up = old->up;
        nnew->down = old;
        old->up->down = nnew;
...
void
Dance::dance_solve (int *solution)
{
  sudoku_dancing_links_solve (board);
  solve ();
  if (!solved(chess))
    assert(0);
